...
func (c *SSHClient) Connect(ws *websocket.Conn) {
	go func() {
		for {
			...
			json.Unmarshal(p, &wdSize)

			if wdSize.High > 0 && wdSize.Width > 0 {
				req := ptyWindowChangeMsg{
					Columns: uint32(wdSize.Width),
					Rows:    uint32(wdSize.High),
					Width:   uint32(wdSize.Width * 8),
					Height:  uint32(wdSize.High * 8),
				}
				_, err := c.channel.SendRequest("window-change", false, ssh.Marshal(&req))
				fmt.Println(err)
				continue
			}
	...
	go func() {
		...
		go func() {
			...
			for {
				...
				if err != nil {
					log.Println(err)
					ws.WriteMessage(1, []byte("\033[SSH Error]"))
					ws.Close()
					return
				}
